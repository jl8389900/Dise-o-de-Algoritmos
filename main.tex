...
Por ejemplo, un algoritmo que se ejecuta en un conmutador de alta velocidad en Internet puede ver un flujo de paquetes volando a través de él, y puede tratar de calcular lo que quiera a medida que este flujo pasa, pero sólo puede realizar una cantidad constante de trabajo computacional en cada paquete, y no puede guardar el flujo con el fin de hacer exploraciones posteriores a través de él. Para estudiar este modelo de cálculo se han desarrollado dos subáreas diferentes de algoritmos: los algoritmos en línea y los algoritmos de flujo de datos.
...
\begin{center}
\includegraphics[scale=0.35]{./Imagenes-Seccion2/2023_08_07_379b6f7f3185549667e3g-06}
\end{center}
...
\includegraphics[scale=0.35]{./Imagenes-Seccion2/2023_08_07_379b6f7f3185549667e3g-06(2)}\\
\includegraphics[scale=0.35]{./Imagenes-Seccion2/2023_08_07_379b6f7f3185549667e3g-07}
...
\begin{center}
\includegraphics[scale=0.35]{./Imagenes-Seccion2/2023_08_07_379b6f7f3185549667e3g-07(1)}
\end{center}
...
\subsection*{Ejercicio Resuelto 1}
Toma la siguiente lista de funciones y ordénalas en orden ascendente de velocidad de crecimiento. Es decir, si la función $g(n)$ sigue inmediatamente a la función $f(n)$ en tu lista, entonces debería darse el caso de que $f(n)$ es $O(g(n))$.
...
& f_{3}(n)=n^{n} \\
& f_{4}(n)=log _{2} n \\
& f_{5}(n)=2^{\sqrt{\log _{2} n}}
\end{aligned}
$$

\subparagraph{Solución} Podemos tratar con las funciones $f_{1}, f_{2}$ y $f_{4}$ muy fácilmente, ya que pertenecen a las familias básicas de exponenciales, polinomios y logaritmos. En particular, por (2.8), tenemos $f_{4}(n)=O\left(f_{2}(n)\right)$; y por (2.9), tenemos $f_{2}(n)=O\left(f_{1}(n)\right)$. Ahora, la función $f_{3}$ no es tan difícil de tratar. Empieza siendo menor que $10^{n}$, pero una vez que $n \geq 10$, entonces claramente $10^{n} \leq n^{n}$. Esto es exactamente lo que necesitamos para la definición de la notación $O(\cdot)$: para todo $n \geq 10$, tenemos $10^{n} \leq c n^{n}$, donde en este caso $c=1$, y por tanto $10^{n}=O\left(n^{n}\right)$.

Por último, llegamos a la función $f_{5}$, que hay que reconocer que es un poco extraña. Una regla útil en estas situaciones es tratar de tomar logaritmos para ver si esto hace las cosas más claras. En este caso, $\log _{2} f_{5}(n)=\sqrt{\log _{2} n}=$ $\left(\log _{2} n\right)^{1 / 2}$. ¿Cómo son los logaritmos de las otras funciones? $\log f_{4}(n)=$ $\log _{2} \log _{2} n$, mientras que $\log f_{2}(n)=\frac{1}{3} \log _{2} n$. Todas ellas pueden verse como funciones de $\log _{2} n$, por lo que utilizando la notación $z=\log _{2} n$, podemos escribir

$$
\begin{aligned}
& \log f_{2}(n)=\frac{1}{3} z \\
& \log f_{4}(n)=log _{2} z \\
& \log f_{5}(n)=z^{1 / 2}
\end{aligned}
$$

Ahora es más fácil ver lo que está pasando. 
En primer lugar, para $z \geq 16$, tenemos $\log _{2} z \leq z^{1/2}$. 
Pero la condición $z \geq 16$ es la misma que $n \geq 2^{16}=65.536$; por lo tanto una vez $n \geq 2^{16}$ tenemos $\log f_{4}(n) \leq \log f_{5}(n)$, y así $f_{4}(n) \leq f_{5}(n)$. 
Así, podemos escribir $f_{4}(n)=O\left(f_{5}(n)\right)$. Del mismo modo tenemos $z^{1 / 2} \leq \frac{1}{3} z$ una vez $z \geq 9$-en otras palabras, una vez que $n \geq 2^{9}=512$. 
Para $n$ por encima de este límite tenemos $\log f_{5}(n) \leq \log f_{2}(n)$ y por lo tanto $f_{5}(n) \leq f_{2}(n)$, y así podemos escribir $f_{5}(n)=O\left(f_{2}(n)\right)$. 
Esencialmente, hemos descubierto que $2^{\sqrt{\log _{2} n}}$ es una función cuya tasa de crecimiento se encuentra en algún lugar entre la de los logaritmos y la de los polinomios.

Como hemos intercalado $f_{5}$ entre $f_{4}$ y $f_{2}$, con esto terminamos la tarea de ordenar las funciones.

\subsection*{Ejercicio Resuelto 2}
Sean $f$ y $g$ dos funciones que toman valores no negativos, y supongamos que $f=O(g)$. Demostrar que $g=\Omega(f)$.

\subparagraph{Solución} Este ejercicio intenta de formalizar la intuición de que $O(\cdot)$ y $\Omega(\cdot)$ son, en cierto sentido, opuestos. De hecho, no es difícil de demostrar; sólo es cuestión de ``desenrollar'' las definiciones.

Se nos da que, para algunas constantes $c$ y $n_{0}$, tenemos $f(n) \leq c g(n)$ para todo $n \geq n_{0}$. Dividiendo ambos lados por $c$, podemos concluir que $g(n) \geq \frac{1}{c} f(n)$ para todo $n \geq n_{0}$. Pero esto es exactamente lo que se requiere para demostrar que $g=\Omega(f)$ : hemos establecido que $g(n)$ es al menos un múltiplo constante de $f(n)$ (donde la constante es $\frac{1}{c}$ ), para todo $n$ suficientemente grande (al menos $n_{0}$ ).
